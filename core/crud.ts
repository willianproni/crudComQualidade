...
function create(content: string): Todo{ //Simula a criação de um dado no banco, aqui estamos criando um arquivo com o dado enviado (POST)
    const todo : Todo = { //Criando um objeto JSON (Lista de tarefas)
...
function update(idUptdate: UUID, contentUpdate: Partial<Todo>) { //Função de Update (PUT)
   let updateTodo;
   ...
   if(hasResultFindId){ // Se encontrar o id
    ...
    if(!updateTodo){ //Verificar se o objeto foi realmente modificado 
...
function updateContentById(id: UUID, content: string){ //Chama a função update
    update(id, {
        content
    })
}
 
function deleteById(id: string) { //função de delete (DELETE)
    const todos = read() //ler a lista de tarefas
    
    const todoWithoutDeleteOne = todos.filter(todo => { //filtrar pela lista a tarefa com o mesmo ID enviado 
        if(todo.id === id){ //caso encontre
            return false //não retornar ele
        }
        return true //retornar o objeto da lista
    })

    fs.writeFileSync(DB_FILE_PATH, JSON.stringify({todos: todoWithoutDeleteOne}, null, 2)); //Rescrever a lista de tarefas sem o id deletado
}

function CLEAR_DB() { //Função para limpar o documento
